...
这个仓库用于管理个人和收藏的 Agent Skill。当前内容以产品需求、交互走查、文本游戏和通用工作流技能为主，同时保留一组收藏技能作为可参考或可迁移的能力库。
...
- 新增技能时，优先保持一个技能一个目录。
- 入口文件统一使用 `SKILL.md`；双语技能可以额外提供 `SKILL.en.md`。
- 大段标准、模板和示例放入 `references/`，避免 `SKILL.md` 过长。
- 可执行工具放入 `scripts/`，并在 `SKILL.md` 中说明调用方式。
- 不把本地私有、实验性或含敏感配置的技能目录提交到 GitHub。
